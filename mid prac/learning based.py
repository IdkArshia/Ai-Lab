...
class LBAgent:
    ...
    def select_act(self, state):
        if random.uniform(0,1)<self.epsilon:
           ...
           return max(self.actions, key=lambda a:self.get_Q_val(state,a)) 
    def learn(self,state, action, reward, next_state):
        old_Q= self.get_Q_val(state, action)
        best_future_Q = max([self.get_Q_val(next_state, a) for a in self.actions])
        self.Q[(state, action)] = old_Q + self.alpha * (reward + self.gamma * best_future_Q - old_Q)
    def act(self, state):
        return self.select_act(state)
...
# Create instances
agent = LBAgent(['Clean the room', 'No action needed'])
